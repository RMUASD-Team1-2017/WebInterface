...
def test_retrieve(page_type, kwargs = {}):
        ...
        c = Client()
        if len(kwargs):
            return c.get(reverse("EmergencyCommon:{}".format(page_type), kwargs = kwargs) ).status_code
        return c.get(reverse("EmergencyCommon:{}".format(page_type)) ).status_code

class PageRetrieveTests(TestCase):
#Simple test to see if we can retrieve the webpages

    def test_dispatch_page(self):
        self.assertEqual(test_retrieve("index"), 200)
